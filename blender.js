 ...
 */
 ...
 module.exports = {
	...
	pipelineEmb : function(table, cb){
		...
		var dataTable = null;
		var dataTable_join = null;
		var dataTablePK = null;
		var dataTableFK = null;

		//AFTER N CALLs (1st param), execute function (2nd param)
		var queryDone = _.after(4, pipeline);
		...
		function pipeline(){
			module.exports.preProcessor(dataTable, function(objectTable){
				module.exports.preProcessor(dataTable_join, function(objectTable_join){
					module.exports.embProcessor(objectTable, objectTable_join, table.emb, dataTableFK, function(objectTable){
		...
		orcl.getTable(table.emb, function(err, result){
			dataTable_join = result;
			queryDone();
		});
	...
	embProcessor : function(table_obj, table_obj_join, table_join_name, table_fk, cb){
		
		var emb_names = {};

		_.each(table_fk.rows, function(row){

			if(row[2] == table_join_name){				

				if(!emb_names[row[1]]){
					emb_names[row[1]] = [];
				}

				emb_names[row[1]].push(row[3]);
			}
		});

		_.each(emb_names, function(value, key){
			emb_names[key] = _.filter(value, function(val){ 
				
				if(emb_names[val] && val != key){
					return false
				}
		...
		_.each(emb_names, function(value, key){
			if(value.length > 0){
				_.each(emb_names, function(value2, key2){
					if(key2 != key){
						_.each(value2, function(val){
							if(val == value){
								emb_names[key2] = _.filter(emb_names[key2], function(eval){return eval != val;});
							}
						});
					}
				});
		...
		async.each(table_obj, function(obj, rowCallback){
			...
			_.each(emb_names, function(value, key){
				blueprint[value[0]] = obj[key];
			})


			obj[table_join_name] = _.where(table_obj_join, blueprint);
